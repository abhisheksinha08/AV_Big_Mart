...
mod2 = lm(Item_Outlet_Sales~Item_Type_Combined + Item_MRP + Num_years + Outlet_SizeHigh + Outlet_SizeMedium + Outlet_Location_TypeTier.1 + Outlet_Location_TypeTier.2 +
Outlet_TypeGrocery.Store + Outlet_TypeSupermarket.Type1 + Outlet_TypeSupermarket.Type2, data = train_new)

pred1 = predict(mod2, newdata = test_new)
...
write.csv(test1[c("Item_Identifier","Outlet_Identifier","Item_Outlet_Sales")], "sub3_lm_all.csv", row.names = FALSE)
